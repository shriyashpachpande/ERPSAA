        ...
        <div className="lg:col-span-3 space-y-6">
          <div className="bg-white rounded-[2.5rem] shadow-sm border border-gray-100 overflow-hidden min-h-[600px] flex flex-col">
            <div className="p-8 border-b border-gray-50 flex items-center justify-between flex-wrap gap-4 bg-gray-50/30">
              ...
              {students.length > 0 && (
                <div className="flex gap-2">
                  <button onClick={() => markAll('Present')} className="px-4 py-2 bg-emerald-50 text-[#10B981] text-[10px] font-black uppercase tracking-widest rounded-xl hover:bg-emerald-100 transition-all">All Present</button>
                  <button onClick={() => markAll('Absent')} className="px-4 py-2 bg-red-50 text-[#EF4444] text-[10px] font-black uppercase tracking-widest rounded-xl hover:bg-red-100 transition-all">All Absent</button>
                </div>
              )}
            </div>
                      ...
                      >
                        <div className="flex items-start justify-between gap-4">
                          ...
                          <div className="flex flex-col gap-1.5">
                            ...
                            <div className="flex gap-1.5">
                              <button onClick={() => handleStatusChange(student._id, 'Late')} className={`flex-1 py-1 px-1.5 rounded-lg text-[8px] font-black uppercase tracking-tighter ${status === 'Late' ? 'bg-amber-500 text-white' : 'bg-white text-gray-300 border border-gray-100'}`}>Late</button>
                              <button onClick={() => handleStatusChange(student._id, 'Excused')} className={`flex-1 py-1 px-1.5 rounded-lg text-[8px] font-black uppercase tracking-tighter ${status === 'Excused' ? 'bg-blue-500 text-white' : 'bg-white text-gray-300 border border-gray-100'}`}>Exc</button>
                            </div>
                          </div>
                        </div>
                      </div>
