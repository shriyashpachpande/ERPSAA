...
const EventApprovalDashboard = () => {
  ...
  const fetchRequests = async () => {
    ...
    try {
      ...
      const url = filter === 'all' ? '/api/events/all' : `/api/events/all?status=${filter}`;
  ...
  const handleAction = async (id, action) => {
    ...
    try {
      ...
      const res = await fetch(`/api/events/${action}/${id}`, {
        method: 'PUT',
        headers: { 'Authorization': `Bearer ${token}` }
      });
      ...
      const data = await res.json();
      if (data.success) {
        toast.success(`Request ${action}d successfully. ${data.message || ''}`);
    ...
    >
      <div className="max-w-7xl mx-auto space-y-8 relative z-10">
        ...
        <header className="flex flex-col md:flex-row md:items-end justify-between gap-6 pb-6 border-b border-slate-200/60">
          <div>
            <h1 className="text-4xl lg:text-5xl font-black tracking-tight mb-2 text-slate-900">Approval Dashboard</h1>
            <p className="text-slate-500 text-lg font-medium">Review and manage student facility booking requests.</p>
          </div>
               ...
               <table className="w-full text-left border-collapse">
                 <thead>
                   <tr className="bg-slate-50 border-b border-slate-200/60 text-xs uppercase tracking-widest text-slate-500 font-black">
                     ...
                     <th className="p-5 font-bold">Facility & Reason</th>
                   ...
                   {requests.map(req => (
                     <tr key={req._id} className="hover:bg-slate-50/50 transition-colors">
                       <td className="p-5">
                         ...
                         <p className="text-xs text-slate-500 font-medium">ID: {req.studentId}</p>
                       </td>
                       <td className="p-5">
                       ...
                       <td className="p-5 text-right">
                         {req.status === 'pending' ? (
                           ...
                           <span className="text-xs font-bold text-slate-400 uppercase">Action Taken</span>
                         )}
                       </td>
                     </tr>
                   ))}
