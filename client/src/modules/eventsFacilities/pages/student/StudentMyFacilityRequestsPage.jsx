...
const StudentMyFacilityRequestsPage = () => {
  ...
  return (
    <div className="min-h-screen lg:p-10 bg-slate-50 text-slate-900 relative overflow-hidden rounded-tl-2xl   bg-transparent"
    >

      <div className="absolute top-0 right-0 w-[500px] h-[500px] blur-[100px] rounded-full pointer-events-none -mt-20 -mr-20 bg-transparent"></div>
      ...
      <div className="max-w-7xl mx-auto relative z-10 space-y-8">
        <header className="flex flex-col md:flex-row md:items-end justify-between gap-6 pb-6 border-b border-slate-200/60">
          <div>
            ...
            <p className="text-slate-500 text-lg font-medium">Track your facility booking applications and live statuses.</p>
          </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            {filtered.map(req => (
              <div key={req._id} className="bg-white border border-slate-200 hover:border-blue-300 rounded-3xl p-6 shadow-sm hover:shadow-xl hover:shadow-blue-500/10 transition-all group relative overflow-hidden flex flex-col justify-between">
                ...
                <div className={`absolute top-0 left-0 w-full h-1 ${req.status === 'approved' ? 'bg-emerald-500' :
                ...
                <div>
                  <div className="flex justify-between items-start mb-4">
                    ...
                    <span className="text-[10px] uppercase font-black tracking-widest text-slate-400 bg-slate-50 px-2 py-0.5 rounded border border-slate-100">{req.categorySlug.replace('-', ' ')}</span>
                  </div>

                  <h3 className="text-xl font-black text-slate-900 mb-1">{req.facilityName}</h3>
...
export default StudentMyFacilityRequestsPage;
